...
class Accounts extends MX_Controller {
    ...
    {
        $direction = $this->input->post('direction', true) ?: 'outbound';
        $result    = $this->accounts_model->insert_block(
            $this->input->post('prefixies', true),
            $accountid,
            $direction
        );
        echo $result;
    ...
    function customer_add_manual_blockpatterns($accountid)
    {
        $data['page_title'] = gettext('Add Block Code');
        $data['accountid']  = $accountid;
        $this->load->view('view_block_manual_prefix', $data);
    }

    function customer_block_prefix_manual($accountid)
    {
        $prefix    = preg_replace('/[^0-9]/', '', $this->input->post('prefix', true));
        $direction = $this->input->post('direction', true) ?: 'outbound';
        ...
        $result = $this->accounts_model->insert_manual_block(
            $prefix,
    ...
    function provider_add_manual_blockpatterns($accountid)
    {
        $this->customer_add_manual_blockpatterns($accountid);
    }

    function provider_block_prefix_manual($accountid)
    {
        $this->customer_block_prefix_manual($accountid);
    }
	...
	function customer_did_country() {
		...
		if ($did_list->num_rows() > 0) {
			...
			foreach ($did_data as $key => $value) {
				$did_arr[$value['product_id']] = $value['number']." (". gettext('Setup Cost') . " : ".$this->common_model->calculate_currency($value['setup_fee'], '', '', true).") (". gettext('MonthlyFee') . " : ".$this->common_model->calculate_currency($value['price'], '', '', true).")";
			}
		}
		$did_info = array(
			"name"  => "free_didlist",
...
//		$entity_type         = strtolower($this->common->default_signup_login_type('', '', $type));
	...
	function customer_generate_cnpj() {
		...
		$this->flux_log->write_log('result', json_encode($accountid));
		...
		$result = $this->accounts_model->consultar($doc, $accountid);
...
//			$this->output->set_status_header(502);
		...
		$this->flux_log->write_log('result', json_encode($result));
		$mapped = $this->accounts_model->map_result_to_account($doc, $accountid, $result['data']);
		echo json_encode(array(
			'ok'     => true,
			'doc'    => $doc,
			'mapped' => $mapped,
